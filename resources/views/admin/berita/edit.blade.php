...
<head>
    ...
    <title>{{ $isEdit ? 'Perbaharui' : 'Tambah' }} Berita</title>
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <main class="flex-1 overflow-auto bg-gray-50 dark:bg-gray-900">
            <div class="bg-white dark:bg-gray-800 shadow-md pl-6 pr-6 py-5 mb-6 flex items-center">
                <h3 class="font-semibold text-gray-800 dark:text-white px-4">
                    {{ $isEdit ? 'Perbaharui' : 'Tambah' }} Berita
                </h3>
            </div>

            <form id="form-berita" action="{{ $action }}" method="POST" enctype="multipart/form-data" class="px-10">
                @csrf
                @if ($isEdit)
                @method('PUT')
                @endif
                ...
                <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="isi_berita" rows="6"
                            class="w-full p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>{{ old('isi_berita', $berita->isi_berita ?? '') }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md">
                            ...
                            <input type="file" name="foto_utama" accept="image/*" onchange="previewFotoUtama(event)" class="w-full p-2 border rounded">
                            <div id="preview-utama" class="mt-3">
                                @if (!empty($berita->foto_utama))
                                <img src="{{ asset($berita->foto_utama) }}" class="h-36 object-cover rounded border">
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md">
                            ...
                            <div id="preview-container" class="flex flex-wrap gap-3 mt-3">
                                ...
                                <div class="relative group" data-index="{{ $index }}">
                                    <img src="{{ asset($foto) }}" class="h-24 object-cover rounded border shadow">
                                    <button type="button"
                            ...
                            <input type="hidden" name="existing_foto_tambahan" id="existing-foto-tambahan" value='@json($fotoTambahanArray)'>
                        </div>
                    </div>

                    <input type="hidden" name="tanggal" id="tanggal-input" value="{{ now() }}">
                    <input type="hidden" name="status" id="status-input" value="{{ old('status', $berita->status ?? 'draft') }}">

                    <div class="flex gap-4 mt-8">
                        <div class="w-1/2">
                            <button type="button" onclick="submitManualForm('publikasi')"
                                class="w-full py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-semibold transition">Publikasi</button>
                        </div>
                        <div class="w-1/2">
                            <button type="button" onclick="submitManualForm('draft')"
                                class="w-full py-3 bg-gray-600 text-white rounded-lg hover:bg-gray-700 font-semibold transition">Simpan Draft</button>
                        </div>
                    </div>
                </div>
            </form>
    ...
    <script>
        ...
        function previewFotoUtama(event) {
            ...
            reader.onload = (e) => {
                ...
                img.classList.add('h-36', 'object-cover', 'rounded', 'border');
        ...
        function renderAllPreviews() {
            ...
            fotoLama.forEach((foto, index) => {
                ...
                const img = document.createElement('img');
                img.src = '/' + foto;
                img.classList.add('h-24', 'object-cover', 'rounded', 'border', 'shadow');
            ...
            renderHiddenInputBaru();
        }

        function renderHiddenInputBaru() {
        ...
        document.getElementById('foto-lain').addEventListener('change', function(e) {
            ...
            if (fotoLama.length + fotoTambahan.length + selected.length > 5) {
        ...
        function submitManualForm(status) {
            const form = document.getElementById('form-berita');
            const formData = new FormData(form);

            formData.set('status', status);
            formData.set('tanggal', new Date().toISOString());
            formData.set('existing_foto_tambahan', JSON.stringify(fotoLama));
